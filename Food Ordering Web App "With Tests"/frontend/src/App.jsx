...
function App() {
    ...
    function addToCart(item, qty) {
        if (qty <= 0) { setMessage('Quantity must be positive'); return; }
        setMessage('');
        setCart(prev => {
    ...
    function updateQuantity(id, quantity) {
        if (quantity <= 0) { setMessage('Quantity must be positive'); return; }
        setMessage('');
    ...
    async function placeOrder() {
        if (!name || !contact) { setMessage('Name and contact required'); return; }
        if (cart.length === 0) { setMessage('Cart empty'); return; }
        setMessage('');
        try {
    ...
    return (
        <div style={{ padding: 20, fontFamily: 'Arial' }}>
            ...
            <ul data-testid="menu-list">
                {displayed.map(it => (
                    <li key={it.id}>
                        <strong>{it.name}</strong> - {it.description} ({it.category}) - €{it.price.toFixed(2)}
                        <div>
            ...
            <ul data-testid="cart-list">
                {cart.map(it => (
                    <li key={it.id}>
                        {it.name} x
                        <input aria-label={'qty-' + it.id} type="number" value={it.quantity}
                               onChange={e => updateQuantity(it.id, parseInt(e.target.value || '0'))} />
            ...
            <div>
                Promo: <input value={promo} onChange={e => setPromo(e.target.value)} />
            </div>
